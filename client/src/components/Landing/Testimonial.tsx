...
const CreateCard: React.FC<{ card: TestimonialCardProps }> = ({ card }) => (
  <div
    className={`p-4 rounded-lg mx-4 shadow-sm hover:shadow-md transition-all duration-300 w-72 shrink-0 
    ${card.bgColor || ""} ${card.hoverColor || "hover:bg-gray-100"}
    `}
  >
    {/* Header */}
    <div className="flex gap-3 items-center">
      <img
        ...
        src={card.image}
        alt={`${card.name || card.author}'s profile`}
      />
      <div className="flex flex-col">
        <div className="flex items-center gap-1">
          <p className="font-medium text-gray-800">
            {card.name || card.author || "Anonymous"}
          </p>
          {/* Verified badge */}
          <svg
            className="mt-0.5"
    ...
    {/* Footer */}
    <div className="flex items-center justify-between text-slate-500 text-xs mt-2">
      <div className="flex items-center gap-1">
        <span>Posted on</span>
        <a
          href="https://x.com"
